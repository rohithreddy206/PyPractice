...
def get_student_with_subjects(student_id: int, request: Request):  # modified
...
def get_student_subjects_only(student_id: int, request: Request):  # modified
    user = getattr(request.state, "user", None)
    if not user:
        raise HTTPException(status_code=401, detail="Not authenticated")
    if user["role"] != "admin" and user["id"] != student_id:
...
def add_subjects_to_student(student_id: int, payload: SubjectIds, request: Request):  # modified
    user = getattr(request.state, "user", None)
    if not user:
        raise HTTPException(status_code=401, detail="Not authenticated")
    if user["role"] != "admin" and user["id"] != student_id:
